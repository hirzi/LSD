...
folded=0
min_sites=100
while getopts b:F:p:r:R:w:o:t:m:q:d:P:f:a:g:S:s:W:X: option
do
case "${option}"
in
b) bamlist=${OPTARG};;
...
## calculate genotype likelihoods, output in binary format (doGlf 1)
	...
	for pop in $(seq 1 ${no_pops}); do
		...
		thetaStat do_stat ${working_dir}/pop${pop}.thetas.idx -win ${window_size} -step ${step_size} &> ${working_dir}/pop${pop}.thetas.idx.pestPG.log
	done
	...
	echo "Preparing calculation of pairwise statistics..."
		for pop in $(seq 1 ${num_pairs}); do
			...
			if [[ ! ${no_pops} = 3 ]]; then
				...
				# realSFS fst stats2 ${working_dir}/${1}.${2}.stats.fst.idx -win ${window_size} -step ${step_size} 2> ${working_dir}/${1}.${2}.globalFST.log | cut -f 2 >> ${working_dir}/${1}.${2}.globalFST
	...
	for pop in $(seq 1 ${no_pops}); do	
		...
		# we need to get mean values across WINDOWS	
		# theta estimators
