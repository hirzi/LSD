...
if __name__ == "__main__":
	...
	if window_size == 0:
		...
		if scaffold_specific == False:			
			...
			# Convert allele frequencies dataframe into input format for LSD-high
			...
			for i in range(len(across_pop_sites_per_scaffold)):
				...
				# Select window region from allele frequency dataframe and convert this into input format for lsd-high. Define region = "single" since we're calculating on a PER scaffold basis.
		...
		if scaffold_specific == False:
			...
			for start_pos, end_pos in zip(start_positions,end_positions):
				...
				# Select window region from allele frequency dataframe and convert this into input format for lsd-high
